...
# local path
test_datafile = "hdfs://soit-hdp-pro-1.ucc.usyd.edu.au/user/zzha7690/test_set_flatten.csv"
train_datafile = "hdfs://soit-hdp-pro-1.ucc.usyd.edu.au/user/zzha7690/train_set_flatten.csv"
...
assembler_test = VectorAssembler(inputCols=test_df.columns[:3072], outputCol="features")
test_vectors_withlabel = assembler_test.transform(test_df).selectExpr("_c3072 as label", "features")

# transfer the train_df in to the dataframe with 2 column label and features so that we can do the further process
assembler_train = VectorAssembler(inputCols=train_df.columns[:3072], outputCol="features")
train_vectors_withlabel = assembler_train.transform(train_df).selectExpr("_c3072 as label", "features")
...
model2 = ovr2.fit(train_vectors_withlabel)
result = model2.transform(test_vectors_withlabel)
